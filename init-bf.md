...
3. **测试账号**（如 your_test_account）
4. **测试密码**（如 your_password_here）

获取来源优先级：
- 命令参数（`/init-bf SPD http://your-test-system.example.com/login your_test_account your_password_here`）
- 项目目录中的 `*测试环境信息*.txt` 文件
- 项目目录名称推断（`D:\BF-CustomerA` → 前缀 NJ，`D:\BF-CustomerB` → 前缀 SPD）
- 交互询问用户
...
将 BF 测试 Agent 安装到项目的 `.claude/agents/` 目录（V2 共 6 个，含图谱子 agent）：
...
安装: bf-e2e-healer.md
安装: bf-ui-explorer.md
安装: bf-graph-agent.md
完成: 安装 6 个 Agent
```

若项目 `.claude/agents/` 目录下已有同名 Agent 文件，直接覆盖（更新到最新版本）。

`bf-graph-agent` 是 V2 关键子 agent：带 Bash 专属跑 `~/.claude/skills/scripts/build_index.py` 维护 SQLite 知识图谱（`{项目}/需求文档/sprint_all/索引/知识图谱.db`）。安装后必须重启 Claude Code 会话才能被发现。

### 第七步：一次性环境自检（V2 新增）

V2 知识图谱依赖 `sqlite3`（标准 CPython 内置）和 FTS5（性能优化，缺则 LIKE 兜底）。在安装 Agent 之后、回到主对话之前，执行以下一次性自检：

```bash
python --version
python -c "import sqlite3; con=sqlite3.connect(':memory:'); con.execute('CREATE VIRTUAL TABLE t USING fts5(x)'); print('FTS5 OK')"
python -c "import docx, openpyxl, pypdf; print('libs OK')"
```

判定规则：
- `python --version` 必须 ≥ `3.8`，否则提示用户升级
- FTS5 自检通过 → 在 CLAUDE.md 记录 `v2_fts5=ok`
- FTS5 自检失败但 `import sqlite3` 通过 → 仍可用，记录 `v2_fts5=fallback_like`（build_index.py 自动走 LIKE 分支）
- `import sqlite3` 失败 → 报错「请使用标准 CPython（Anaconda/conda 也行，但需带 sqlite3）」，中止 V2 流程
- `docx / openpyxl / pypdf` 任一缺失 → 提示 `pip install python-docx openpyxl pypdf`（与 V1 一致）

自检结果写入 CLAUDE.md 的「项目信息」段，例如：
```markdown
- v2_fts5: ok
- v2_sqlite3: ok
- v2_build_index: ~/.claude/skills/scripts/build_index.py
```

后续 `bf-graph-agent` 跑 `build_index.py` 时，脚本内部也会再自检一次（防御），但 CLAUDE.md 的记录用于让主对话在不进 agent 的情况下也能判断是否需要降级提示。
